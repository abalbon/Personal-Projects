...
int main()
{
  ...
  {
    cout << vectorOfElements.at(zi).getLex() << " ";
  }

  cout << endl << "--------------------------------------";
  cout << endl << "Syntax Analysis" << endl;
  cout << "--------------------------------------" << endl;

  // Creates a recursive decent parser class using the vector of elements
  // that the lexer created
  RDP expressionToParse(vectorOfElements);
  expressionToParse.parse();      // parse the expression

  cout << "\n\nEnd Program\n\n";  // Output statement to signify the end
